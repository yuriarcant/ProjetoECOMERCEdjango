...
from .models import *
import uuid
from .utils import filtrar_produtos, preco_minimo_maximo, ordenar_produtos, enviar_email_compra, exporta_csv
    ...
    if request.user.is_authenticated:
        ...
        if request.COOKIES.get('id_sessao'):
            ...
            context = {"cliente_existente": False,
                       "itens_pedidos": None, "pedido": None, }
            return render(request, 'carrinho.html', context)
    pedido, criado = Pedido.objects.get_or_create(
        cliente=cliente, finalizado=False)
    itens_pedidos = ItensPedido.objects.filter(pedido=pedido)
    context = {"itens_pedidos": itens_pedidos,
...
def criar_conta(request):
    ...
    if request.method == "POST":
        dados = request.POST.dict()
        if 'email' in dados and 'senha' in dados and 'confirmacao_senha':
            email = dados.get('email')
            senha = dados.get('senha')
...
def gerenciar_loja(request):
    if request.user.groups.filter(name="equipe").exists():
        pedido_finalizado= Pedido.objects.filter(finalizado= True)
        qtde_pedidos= len(pedido_finalizado)
        faturamento=sum([pedido.preco_total for pedido in pedido_finalizado])
        qtde_produtos=sum([pedido.quantidade_total for pedido in pedido_finalizado])
...
@login_required
def exporta_relatorio(request, relatorio):
    if request.user.groups.filter(name="equipe").exists():
        if relatorio == 'pedido':
            informaçoes = Pedido.objects.filter(finalizado=True)
        if relatorio == 'cliente':
            informaçoes = Cliente.objects.all()
        if relatorio == 'endereco':
            informaçoes = Endereco.objects.all()
        return exporta_csv(informaçoes)
    else:
